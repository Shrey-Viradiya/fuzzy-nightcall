...
class Discriminator(torch.nn.Module):
    def __init__(self, discriminator_feature_map_size = 64, input_channel=3):
        ...
        self.main = torch.nn.Sequential(
            # input is (input_channel) x 256 x 256
            torch.nn.Conv2d(input_channel, discriminator_feature_map_size, 4, 2, 1, bias=False),
            torch.nn.LeakyReLU(0.2, inplace=True),
            # state size. (discriminator_feature_map_size) x 128 x 128
            ...
            torch.nn.LeakyReLU(0.2, inplace=True),
            # state size. (discriminator_feature_map_size*2) x 64 x 64
            ...
            torch.nn.LeakyReLU(0.2, inplace=True),
            # state size. (discriminator_feature_map_size*4) x 32 x 32
            ...
            torch.nn.LeakyReLU(0.2, inplace=True),
            # state size. (discriminator_feature_map_size*8) x 16 x 16
            torch.nn.Conv2d(discriminator_feature_map_size * 8, discriminator_feature_map_size * 16, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(discriminator_feature_map_size * 8),
            torch.nn.LeakyReLU(0.2, inplace=True),
            # state size. (discriminator_feature_map_size*16) x 8 x 8
            torch.nn.Conv2d(discriminator_feature_map_size * 16, discriminator_feature_map_size * 32, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(discriminator_feature_map_size * 8),
            torch.nn.LeakyReLU(0.2, inplace=True),            
            # state size. (discriminator_feature_map_size*32) x 4 x 4
            torch.nn.Conv2d(discriminator_feature_map_size * 8, 1, 4, 1, 0, bias=False),
            torch.nn.Sigmoid()
        )
